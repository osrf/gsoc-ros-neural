...
#include <mindwave_msgs/ExecuteCartesianIKTrajectory.h>
#include <vector>
...
public:
  IKTrajectoryExecutor(){
    ...
    //service = node.advertiseService(ARM_IK_NAME,
    //  &IKTrajectoryExecutor::execute_cartesian_ik_trajectory, this);
  ...
  bool run_ik(geometry_msgs::PoseStamped pose, double start_angles[7], 
    ...
    ik_request.ik_request.ik_seed_state.joint_state.name.push_back("wrist_2_joint");
    ik_request.ik_request.ik_seed_state.joint_state.name.push_back("wrist_3_joint");
  ...
  void get_current_joint_angles(double current_angles[7]){
    ...
    pr2_controllers_msgs::JointTrajectoryControllerStateConstPtr state_msg = 
      ros::topic::waitForMessage<pr2_controllers_msgs::JointTrajectoryControllerState>("arm_controller/state");
  ...
  bool execute_cartesian_ik_trajectory(
         mindwave_msgs::ExecuteCartesianIKTrajectory::Request &req,
         mindwave_msgs::ExecuteCartesianIKTrajectory::Response &res){
    ...
    for(i=0; i<trajectory_length; i++){
      ...
      success = run_ik(stamped_pose, last_angles, trajectory_point, "wrist_3_joint");
